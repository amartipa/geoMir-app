...
<form method="post" action="{{ route('posts.store') }}" enctype="multipart/form-data" class="bg-white p-8 rounded shadow-md">
    ...
    <div class="flex justify-between">
        <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="button">{{__('Create')}}</button>
        <button type="reset" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="button">{{__('Reset')}}</button>
    </div>
</form>
